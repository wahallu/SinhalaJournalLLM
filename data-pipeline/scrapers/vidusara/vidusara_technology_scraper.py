...
SOURCE       = "vidusara"
CATEGORY     = "Technology"
CATEGORY_URL = "https://www.vidusara.lk/category/features/technology/"

OUTPUT_DIR    = "output"
OUTPUT_FILE   = "vidusara_technology.jsonl"
PROGRESS_FILE = "vidusara_technology_progress.json"
    ...
    if not links:
        for a in soup.select("a.pcbg-bgoverlay"):
            ...
            if href and href not in seen:
                seen.add(href)
                links.append(href)
    # Fallback: any article-like link under /features/
    if not links:
        for a in soup.find_all("a", href=True):
            href = a["href"].strip()
            if re.match(r"https?://www\.vidusara\.lk/features/(technology/)?\d+", href) and href not in seen:
                seen.add(href)
                links.append(href)
    return links
...
async def main():
    ...
    try:
        ...
        progress["stopped_reason"] = (
            "Interrupted by user or signal" if _shutdown else "Completed all pages"
        )
        progress["stopped_at"] = datetime.datetime.now().isoformat()
        save_progress(progress)

        print()
        print(f"🎉 DONE — Vidusara Technology News")

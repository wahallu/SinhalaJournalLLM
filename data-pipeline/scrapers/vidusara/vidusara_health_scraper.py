...
SOURCE       = "vidusara"
CATEGORY     = "Health"
CATEGORY_URL = "https://www.vidusara.lk/category/features/health/"

OUTPUT_DIR    = "output"
OUTPUT_FILE   = "vidusara_health.jsonl"
PROGRESS_FILE = "vidusara_health_progress.json"
...
def extract_article_links(html: str) -> list[str]:
    ...
    if not links:
        for a in soup.find_all("a", href=True):
            href = a["href"].strip()
            if re.match(r"https?://www\.vidusara\.lk/features/(health/)?\d+", href) and href not in seen:
                seen.add(href)
                links.append(href)
    return links
...
async def main():
    ...
    try:
        ...
        print()
        print(f"🎉 DONE — Vidusara Health News")

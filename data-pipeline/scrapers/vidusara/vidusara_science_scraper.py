...
# Scrape from both Science News AND Physics — both categorised as "Science"
CATEGORY_URLS = [
    "https://www.vidusara.lk/category/science-news/",
    "https://www.vidusara.lk/category/features/physics/",
]

OUTPUT_DIR    = "output"
OUTPUT_FILE   = "vidusara_science.jsonl"
PROGRESS_FILE = "vidusara_science_progress.json"
...
def extract_article_links(html: str) -> list[str]:
    ...
    for a in soup.select("h3.pcbg-title a"):
        href = a.get("href", "").strip()
        if href and href not in seen:
            seen.add(href)
            links.append(href)
    # Fallback: overlay links with title attribute
    if not links:
        for a in soup.select("a.pcbg-bgoverlay"):
            href = a.get("href", "").strip()
            if href and href not in seen:
                seen.add(href)
                links.append(href)
    # Fallback: any article-like link
    if not links:
        for a in soup.find_all("a", href=True):
            href = a["href"].strip()
            if (re.match(r"https?://www\.vidusara\.lk/(science-news|features)/\d+", href)
...
def extract_article(html: str):
    """Extract headline, body, and date from article page."""
    soup = BeautifulSoup(html, "lxml")

    # ── Headline ──
    headline = ""
    h1 = soup.find("h1", class_=lambda c: c and "post-title" in c)
    if h1:
    ...
    # ── Date ──
    date = ""
    time_el = soup.find("time", class_=lambda c: c and "entry-date" in c)
    if time_el:
...
def make_run_cfg(timeout: int = PAGE_TIMEOUT) -> CrawlerRunConfig:
    return CrawlerRunConfig(
        exclude_external_links=True, page_timeout=timeout,
...
_BROWSER_CRASH_PHRASES = (
    ...
    "browsercontext.new_page", "browser.new_context",
)


async def fetch_with_retry(crawler_holder, url, cfg, new_crawler_fn, kill_crawler_fn, label=""):
...
async def main():
    ...
    try:
        ...
        for source_url in CATEGORY_URLS:
            if _shutdown:
                break

            source_key = source_url  # use URL as progress key
            src_progress = progress.get("sources", {}).get(source_key, {"last_page": 0, "done": False})
            if src_progress.get("done"):
                print(f"\n⏭️  Skipping already-completed source: {source_url}")
                continue

            resume_page = src_progress.get("last_page", 0) + 1
            print(f"\n{'='*60}")
            ...
            while True:
                ...
                listing_url = (
                    source_url if page_num == 1
                    else f"{source_url}page/{page_num}/"
                )
                ...
                if not res or not res.html:
                    print(f"   ⚠️  Page {page_num} returned no HTML — ending this source.")
                    break

                if res.status_code and res.status_code == 404:
                    print(f"   🏁 Page {page_num} returned 404 — reached the end.")
                    break
                ...
                for idx, article_url in enumerate(article_links, 1):
                    ...
                    if not headline or not content or len(content.split()) < 5:
                        await asyncio.sleep(BETWEEN_ARTICLES)
                        continue
                ...
                if "sources" not in progress:
                    progress["sources"] = {}
                progress["sources"][source_key] = {"last_page": page_num, "done": False}
                progress["total_articles"] = total_articles
                save_progress(progress)
                page_num += 1

            # Mark source as done if not interrupted
            if not _shutdown:
                if "sources" not in progress:
                    progress["sources"] = {}
                progress["sources"][source_key] = {"last_page": page_num - 1, "done": True}
                save_progress(progress)
        ...
        progress["stopped_reason"] = (
            "Interrupted by user or signal" if _shutdown else "Completed all sources"
        )
        progress["stopped_at"] = datetime.datetime.now().isoformat()
        save_progress(progress)

        print()
        print(f"🎉 DONE — Vidusara Science News (Science News + Physics)")

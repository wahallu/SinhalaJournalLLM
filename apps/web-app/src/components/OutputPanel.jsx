    ...
    <div id="output-panel" className="mt-6">
      <div className="flex items-center justify-between mb-2.5">
        <span className="text-xs font-semibold text-gray-400 uppercase tracking-widest">
          ...
          {output.model_used && (
            <span
              className={`ml-2 normal-case tracking-normal font-medium text-[10px] px-1.5 py-0.5 rounded
                ${output.model_used === 'sinllama'
                  ? 'bg-green-50 text-green-600'
                  : output.model_used === 'mock'
                    ? 'bg-amber-50 text-amber-600'
                    : 'bg-blue-50 text-blue-600'}`}
              title={output.model_used === 'sinllama'
                ? 'Generated by the fine-tuned SinLlama model'
                : output.model_used === 'mock'
                  ? 'Offline fallback — model server unavailable'
                  : 'Generated by hosted fallback model'}
            >
              {output.model_used === 'sinllama' ? 'SinLlama' : output.model_used}
            </span>
          )}
        </span>
        <button

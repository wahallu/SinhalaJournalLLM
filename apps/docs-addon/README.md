...
## 5. Testing Against a Local Backend (ngrok)

Google's servers execute the Apps Script code. They cannot resolve `http://localhost:8000` on your local computer. To test your local backend:

1. Expose your local FastAPI server to the internet using **ngrok**:
   ```bash
   ngrok http 8000
   ```
2. Copy the public forwarding HTTPS URL generated by ngrok (e.g., `https://a1b2-34-56-78.ngrok-free.app`).
3. Open the **Settings** tab in the SinAI Docs Sidebar.
4. Set the **API Base URL** to your ngrok URL with the version suffix:
   `https://a1b2-34-56-78.ngrok-free.app/api/v1`
5. Click **සැකසුම් සුරකින්න (Save Settings)**. The connection badge in the header will turn green and read **Connected**.
6. Highlight Sinhala text in your document, click the **Refresh selection (🔄)** button, and run your tools!

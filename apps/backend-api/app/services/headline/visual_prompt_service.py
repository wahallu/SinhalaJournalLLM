...
Reads a Sinhala news article (and optional headline) via the Gemini API and
...
async def generate_visual_prompt(article_text: str, headline: str = "") -> str:
    """
    Call the Gemini API to produce a detailed image-generation prompt.
    ...
    logger.info(
        "Generating visual prompt via Gemini (article length=%d chars)", len(article_text)
    )

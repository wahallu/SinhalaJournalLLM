...
POST /image/generate  —  Generate an image from a visual prompt via Cloudflare Workers AI
                         (Flux 1 Schnell model). Returns a base64 PNG data URL.
"""
...
async def generate_image_endpoint(payload: ImageGenerationRequest):
    ...
    Proxies the request to Cloudflare Workers AI (Flux 1 Schnell) so the API
    token never leaves the backend.

    Returns a base64-encoded PNG as a data URL that the browser can render
